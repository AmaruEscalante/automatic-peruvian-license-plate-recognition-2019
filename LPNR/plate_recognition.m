function [plate] = plate_recognition(input_image,model,steps)
%% Read Image
I = input_image;
I = rgb2gray(I);
% Detect Plates in Scene
%figure; imshow(I); title('Original Image');
%steps = 0; % steps 0 to not show
nomatches = 0;
if steps == 1
    figure; imshow(I); title('Original Image');
...
if strcmp(model,'1') == 1
    load KNNMdl
    Mdl = KNNMdl;
elseif strcmp(model,'2') == 1
    load OptKNNMdl
    Mdl = OptKNNMdl;
elseif strcmp(model,'3') == 1
    load svmModel
    Mdl = svmMdl; %Multiple Class SVM
elseif strcmp(model,'4') == 1
    load treeSVM
    Mdl = SVMModels; %Tree SVM
elseif strcmp(model,'5') == 1
    load SVMMulticlassMdl
    Mdl = Mdl; %Tree SVM
end
...
for k = 1:length(match)
    ...
    if strcmp(model,'4') == 1
        charClass = predictSVM(Mdl,flatt);
    else
        charClass = predict(Mdl,flatt);
    end
    plate(k) = charClass;
